...
#include <direct.h>
#include <malloc.h>
//#include "fmod/samples/fmod/resource.h"
...
AdjustWindowRect(&r,WS_OVERLAPPEDWINDOW,false);
    ...
    // Create the application's window
     hWnd = CreateWindow( "crapcrap", "crap crap",
                              WS_OVERLAPPEDWINDOW, 0,0,r.right-r.left,r.bottom-r.top,
...
bool StartFrame(DWORD clearcol)
{
...
{
	IDirect3DTexture9 *tex = NULL;
	D3DXCreateTexture( g_pd3dDevice, width, height, 1, D3DUSAGE_DYNAMIC, D3DFMT_A8B8G8R8, D3DPOOL_DEFAULT, &tex );
	return tex;
}
...
char* LockTexture( void* texT, int& pitch  )
{
	...
	IDirect3DTexture9 *tex = (IDirect3DTexture9*)texT;
	D3DLOCKED_RECT rect;
	tex->LockRect( 0, &rect, NULL, 0 );
	pitch = rect.Pitch;
	return (char*)rect.pBits;
}
...
void DrawSprite(float xcentre, float ycentre, float xsize, float ysize, float angle, DWORD col )
{
	float c=cosf(angle);
	float s=sinf(angle);
